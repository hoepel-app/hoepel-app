...
export abstract class Collection {
  constructor(
    ...
    protected permissions: CollectionPermissions
  ) {
  ...
  protected sanityCheckCollectionPermissions(
    permissions: CollectionPermissions
  ): void {
    const assertPermissionDoesNotInclude = (
      permission: string | null,
      operationName: 'read' | 'write' | 'update' | 'delete'
    ): void => {
      if (permission == null) {
        return
      }
