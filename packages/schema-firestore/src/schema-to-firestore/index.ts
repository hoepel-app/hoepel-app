...
export const getFirestoreCollections = (
  ...
  visit(schema, {
    ...
    (el: Collection | TypeDefinitionNode) => el instanceof Collection
  )

const visitObjectTypeDefinition = (
  ...
  const collectionDetails = directiveVisitResult?.directives?.filter(
    (el: CollectionDetails | null) => el != null
  )?.[0]
...
const visitObjectTypeDirective = (
  node: DirectiveNode
  ...
  if (node.name.value === 'firestore') {
    ...
    if (type === null || name === null) {
      throw new Error(
        'type is null or name is null - this should not be possible (non-null in schema)'
      )
    }
...
const collectionDetailsToCollection = (
  details: CollectionDetails
): Collection => {
  switch (details.type) {
    case 'INDEXED_BY_TENANT':
      return new IndexedByTenantCollection(
        details.name,
        {
          readPermission: details.readPermission,
          writePermission: details.writePermission,
          updatePermission: details.allowUpdate
            ? details.writePermission
            : null,
          deletePermission: details.allowDelete
            ? details.writePermission
            : null,
        },
    ...
    case 'DOC_PER_TENANT':
      if (details.readPermission == null || details.writePermission == null) {
        throw new Error(
          'For a DOC_PER_TENANT collection, both readPermission and writePermission need to be set'
        )
      }
...
const getCollectionType = (
  ...
  const isValidCollectionType = (value: string): boolean =>
    ['INDEXED_BY_TENANT', 'DOC_PER_TENANT', 'DOC_PER_USER'].includes(value)

  if (argument?.kind === 'EnumValue' && isValidCollectionType(argument.value)) {
    return argument.value as CollectionType
  } else {
    throw new Error(
      `Missing or invalid value for Firestore collection type (argument: ${argument})`
    )
  }
}

const getArgumentAsStringValue = (
  node: DirectiveNode,
  argName: string
): string | null => {
  const argument = node.arguments?.find(arg => arg.name.value === argName)
...
const getArgumentAsBooleanValue = (
  node: DirectiveNode,
  argName: string
): boolean | null => {
  const argument = node.arguments?.find(arg => arg.name.value === argName)
